...
public class GameOverUIManager : MonoBehaviour
{
    ...
    public void ShowGameOver(bool isWin, bool isMultiplayer)
    {
        ...
        if (resultText != null)
        {
            ...
            {
                resultText.text = "GameOver"; // 싱글에서는 결과 텍스트 숨김
            }
        }
        else
        {
        ...
        //  게임 오버 시 보스 UI도 숨김 처리
        if (BossUIManager.Instance != null)
        {
            Debug.Log("[GameOverUI] BossUIManager 숨김 처리");
            BossUIManager.Instance.Hide();
        }
        //InGameUpgradeUI 비활성화
        if (InGameUpgradeUIManager.Instance != null)
        {
            Debug.Log("[GameOverUI] InGameUpgradeUI 숨김 처리");
